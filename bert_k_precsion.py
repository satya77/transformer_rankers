...
def run_experiment(args):
    ...
    if args.task in ["mantis", "msdialog", "ubuntu_dstc8"]:
        add_turn_separator = (
                    args.task != "ubuntu_dstc8")  # Ubuntu data has several utterances from same user in the context.
        train = preprocess_crr.read_crr_tsv_as_df(args.data_folder + args.task + "/train.tsv", args.sample_data,
                                                  add_turn_separator)
        valid = preprocess_crr.read_crr_tsv_as_df(args.data_folder + args.task + "/valid.tsv", args.sample_data,
                                                  add_turn_separator)
    ...
    ## Similar Question Retrieval and Passage Retrieval
        ...
        train, valid = preprocess_scisumm_ranked.transform_to_dfs("../data/Training-Set-2019/Task1/satya_input.tsv")
    ...
    if args.test_negative_sampler == 'random':
        ...
        ns_val = negative_sampling.BM25NegativeSamplerPyserini(
            ...
            args.num_ns_eval, args.data_folder + "/" + args.task + "/anserini_valid/", args.sample_data,
    ...
    if args.predict_with_uncertainty_estimation:
        ...
        for metric, v in res.items():
            logging.info(
                "Test (w. dropout and {} foward passes) {} : {:4f}".format(args.num_foward_prediction_passes, metric,
                                                                           v))
        ...
        for i, f_pass_preds in enumerate(foward_passes_preds):
            ...
            preds_df.to_csv(args.output_dir + "/" + args.run_id + "/predictions_with_dropout_f_pass_{}.csv".format(i),
                            index=False)
